...
let isGetWalletInfo = false;

// Function to call the '/api/copytrade' endpoint
export const getMyWalletInfo = async (phantomWalletKey: string): Promise<any> => {
  try {
    if (isGetWalletInfo) return; // Prevent multiple API calls
    ...
    let response = await axios.post(
      'http://localhost:2000/api/copytrade/getMyWalletInfo', 
    ...
    return response.data.walletOverview;
  } catch (error) {
    ...
    if (errorStatus.response.status === 403) {
      ...
      if (newAccessToken) {
        ...
        return await getMyWalletInfo(phantomWalletKey);
      } else {
